          ...
          <div id="menu-mobile">
            ...
            <a-menu v-model="currentPage" mode="inline" class="menu">
              <a-sub-menu v-if="$navs.length" key="category">
                ...
                <a-menu-item-group>
                  <a-menu-item v-for="nav in $navs" :key="nav.key" @click="changeVisible">
                    <a v-if="$route.path === nav.link" href="javascript:;"> {{ nav.text }} </a>
              ...
              <a-menu-item v-show="timeline" key="timeline" @click="changeVisible">
                <router-link to="/timeline.html"> {{ $l('timeline') }} </router-link>
              </a-menu-item>
              <a-menu-item v-show="links" key="links" @click="changeVisible">
                <router-link to="/links.html"> {{ $l('links') }} </router-link>
              </a-menu-item>
      ...
      <a-row>
        <a-col :xxl="4" :xl="5" :lg="5" :md="5" :sm="24" :xs="24">
          <router-link class="logo" to="/">
            ...
            <svg v-if="nameplate" :width="nameplate.width" :height="nameplate.height" xmlns="http://www.w3.org/2000/svg">
              <g>
                <text
            ...
            >
              <template slot="dataSource">
                <a-select-opt-group v-for="group in searchSource" :key="group.key" class="select-group">
                  <span slot="label" class="group-title"> {{ group.title }} </span>
                  <template v-if="group.title === $l('tags')">
                    <a-select-option v-for="opt in group.children" :key="opt.key" :value="searchKeyword" class="select-option-tag">
                      <router-link :to="opt.path">
                        <a-tag> {{ opt.title }} </a-tag>
                      </router-link>
                    </a-select-option>
                  </template>
                  <template v-else>
                    <a-select-option v-for="opt in group.children" :key="opt.key" :value="searchKeyword" class="select-option">
                      <router-link :to="opt.path"> {{ opt.title }} </router-link>
                    </a-select-option>
                  </template>
                </a-select-opt-group>
                ...
                <a-select-option key="v-search-more" :value="searchKeyword" class="search-more">
                  <div class="text" @click="handleMore"> {{ $l('more') }}</div>
                </a-select-option>
              </template>
            </a-auto-complete>
          </div>
          <div class="navigation">
              ...
              <a-sub-menu v-if="$navs.length" key="category">
                ...
                <a-menu-item-group>
                  <a-menu-item v-for="nav in $navs" :key="nav.key">
                    <a v-if="$route.path === nav.link" href="javascript:;"> {{ nav.text }} </a>
                    <router-link v-else :to="nav.link"> {{ nav.text }} </router-link>
                  </a-menu-item>
                </a-menu-item-group>
              </a-sub-menu>
            </a-menu>
            <a-menu v-model="currentPage" mode="horizontal" class="menu">
              ...
              <a-menu-item v-show="about" key="about">
                <router-link to="/about.html"> {{ $l('about') }} </router-link>
              </a-menu-item>
            </a-menu>
          </div>
        </a-col>
