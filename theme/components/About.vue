<template>
  <div id="about">
    <div ref="author" class="author">
      <div class="wrapper">
        <img :src="avatar" :alt="$l('title')">
        <h1>{{ author }}</h1>
        <div v-if="Object.keys(social).length" class="social">
          <span v-for="(v, k) in social" :key="k">
    ...
    <div class="readme">
      <div ref="readmeWrapper" class="wrapper">
        <a-icon type="caret-down" theme="filled" @click="handleReadme"/>
      </div>
...
export default {
  ...
  data () {
    return {
      readme: true,
      avatar: require('@theme/assets/avatar.jpg'),
      author: 'ZYS',
  ...
  methods: {
    ...
    handleLoad () {
      if (this.$refs.readmeWrapper) {
        ...
        this.$refs.readmeWrapper.style.visibility = 'none'
        this.readme = true
      }
    },
    handleReadme () {
