      ...
      <div v-for="(item, index) in links" :key="index" class="wrapper">
        ...
        <a-row>
          <a-col v-for="list in item.lists" :key="list.link" class="col" :xs="24" :sm="12" :md="8" :lg="6">
            <div class="card" :style="{ borderTop: `3px solid ${list.color}` }">
              <a :href="list.link" class="title" target="_blank" rel="noopener noreferrer"> {{ list.title }} </a>
              <div class="subtitle"> {{ list.subtitle }} </div>
              <a :href="list.link" class="logo" target="_blank" rel="noopener noreferrer"
                :style="{ backgroundImage: `url(${ list.logo })`, backgroundColor: list.color }"
              />
            </div>
          </a-col>
        </a-row>
