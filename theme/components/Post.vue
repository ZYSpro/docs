<template>
  <div id="post">
    ...
    <div v-if="getCatalogs.length" class="catalog">
      ...
      <a-drawer placement="right" :title="$page.title" :closable="false" :visible="catalog.visible" @close="catalog.visible = false">
        <a-anchor :affix="false" :bounds="200" :wrapper-style="{maxHeight: 'auto'}">
          <a-anchor-link v-for="(catalog, index) in getCatalogs" :key="index" :href="`#${catalog.slug}`" :title="catalog.title">
            <template v-if="hasOwn(catalog, 'children') && catalog.children.length">
              <a-anchor-link v-for="(child, index) in catalog.children" :key="index" :href="`#${child.slug}`" :title="child.title"/>
            </template>
          </a-anchor-link>
        </a-anchor>
      </a-drawer>
